...
async function createFile(projectRoot: string, relativePath: string, content: string = '') {
...
async function createSymlink(projectRoot: string, target: string, linkPath: string) {
...
describe('canResolve() comprehensive tests', () => {
    ...
    describe('Package imports (should not be handled by our transform)', () => {
        ...
        test('should handle package imports in node_modules', async () => {
            await createFile(project.root, 'node_modules/fake-package/package.json',
                JSON.stringify({ name: 'fake-package', main: 'index.js', type: 'module' }));
    ...
    describe('Performance and reliability', () => {
        test('should handle concurrent resolution requests', async () => {
            ...
            const promises = files.map(file =>
                Promise.resolve(canResolve(`./${file}`, importer))
            );
