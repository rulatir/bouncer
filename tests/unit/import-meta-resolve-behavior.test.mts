...
function canResolveWithImportMeta(specifier: string, importer: string): boolean {
    try {
        execSync(
...
function canActuallyImport(specifier: string, importer: string): boolean {
    try {
    ...
    test('directory with index vs direct file priority', async () => {
        ...
        try {
            const resolved = execSync(
                `node --input-type=module -e "console.log(await import.meta.resolve('./plugins', '${importer}'))"`,
